...
class PaaTPNew {
  ...
  public static void createGraph() {
    ...
    for(int i = 0; i < solutionVector.length; i++) {
      ...
      x1 = trapezoidais[solutionVector[i]].getX1() * 8;
      x2 = trapezoidais[solutionVector[i]].getX2() * 8;
      x3 = trapezoidais[solutionVector[i]].getX3() * 8; 
      ...
      if(tmp1 >= tmp2) {  
          ...
          if(x3 <= tmp1) { 
            ...
            g.drawLine(lastPos1 - x3, 0, lastPos1 + x2, 0, "BLACK", 0.003);    //x3+x2
            g.drawLine(lastPos1 - x3, 0, lastPos1, 100, "BLACK", 0.003); //x3 <--> x1
            g.drawLine(lastPos1 + x2 , 0, x1 + lastPos1 , 100, "BLACK", 0.003);  //x1 <--> x2
  
            lastPos1 = lastPos1 + x1;
            lastPos2 = lastPos1 + x2 - x1;
          }else{
            ...
            g.drawLine(lastPos2, 0, lastPos2 + x2 + x3, 0, "BLACK", 0.003);    //x3+x2
            g.drawLine(lastPos2, 0,lastPos2 + x3, 100, "BLACK", 0.003); //x3 <--> x1
            g.drawLine(lastPos2 + x2 + x3 , 0, x1 + x3 + lastPos2 , 100, "BLACK", 0.003);  //x1 <--> x2
